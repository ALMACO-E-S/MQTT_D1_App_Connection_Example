...
MQTTConnection mqtt = new();
mqtt.D1Response += D1Response;
mqtt.D1Readings += D1Readings;
JSON_Response d1Response = new();
JSON_D1Readings d1Readings = new();
...
while (connected)
{
  ...
  {
    case "hide":
      JSON_Command hide = new()
      {
        timestamp = DateTime.Now,
        command_string = "hide"
      };
      var hideJSON = JsonSerializer.Serialize(hide);
      await mqtt.MqttPublish("d1-command", hideJSON);
      break;
    case "show":
      JSON_Command show = new()
      {
        timestamp = DateTime.Now,
        command_string = "show"
      };
      var showJSON = JsonSerializer.Serialize(show);
